package com.blaizmiko.integration
...
class AuthLifecycleTest : StringSpec({
    ...
    fun testApp(block: suspend ApplicationTestBuilder.() -> Unit) = testApplication {
        environment {
            config = io.ktor.server.config.MapApplicationConfig(
                ...
                "ktor.application.modules.0" to "com.blaizmiko.ApplicationKt.module",
    ...
    "full auth lifecycle: register -> access -> refresh -> access -> logout -> refresh fails" {
        testApp {
            ...
            val profileResponse = client.get("/api/v1/auth/me") {
            ...
            val profileResponse2 = client.get("/api/v1/auth/me") {
            ...
            val oldRefreshResponse = client.post("/api/v1/auth/refresh") {
                contentType(ContentType.Application.Json)
                setBody(RefreshRequest(tokens.refreshToken))
            }
            ...
            val reuseDetectedRefresh = client.post("/api/v1/auth/refresh") {
                contentType(ContentType.Application.Json)
            ...
            val postLogoutRefresh = client.post("/api/v1/auth/refresh") {
                contentType(ContentType.Application.Json)
    ...
    "registration with invalid data returns 400" {
        testApp {
            ...
            // Bad email
            val badEmail = client.post("/api/v1/auth/register") {
                contentType(ContentType.Application.Json)
            ...
            // Weak password
            val weakPassword = client.post("/api/v1/auth/register") {
                contentType(ContentType.Application.Json)
            ...
            // Invalid username
            val badUsername = client.post("/api/v1/auth/register") {
                contentType(ContentType.Application.Json)
    ...
    "login with wrong credentials returns 401" {
        testApp {
            ...
            // Non-existent email
            val noUser = client.post("/api/v1/auth/login") {
                contentType(ContentType.Application.Json)
    ...
    "profile update and password change" {
        testApp {
            ...
            // Update username
            val updateResponse = client.patch("/api/v1/auth/me") {
                bearerAuth(tokens.accessToken)
                contentType(ContentType.Application.Json)
            ...
            // Change password
            val changePassResponse = client.put("/api/v1/auth/me/password") {
                bearerAuth(tokens.accessToken)
                contentType(ContentType.Application.Json)
            ...
            val oldPassLogin = client.post("/api/v1/auth/login") {
                contentType(ContentType.Application.Json)
    ...
    "duplicate email registration returns 409" {
        testApp {
            ...
            val dupeResponse = client.post("/api/v1/auth/register") {
                contentType(ContentType.Application.Json)
